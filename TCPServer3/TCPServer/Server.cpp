...
//    serveraddr.sin_addr.s_addr = htonl(INADDR_ANY); //지역IP 주소 ,address,INADDR_ANY=>모든 주소에대해 동작하것다.
...
//		//접속대기큐에 접속해있는 클라이언트의 정보를 얻어와 통신소켓을 반환해준다.
//		client_sock = accept( //통신소켓의 핸들,통신소켓을 얻어온다
//            listen_sock, //대기 소켓
...
//   
//   // bind()
//   SOCKADDR_IN serveraddr; // 2바이트2개 4바이트짜리 하나
//   ZeroMemory(&serveraddr, sizeof(serveraddr));
//   serveraddr.sin_family = AF_INET; // 주소체계
//   serveraddr.sin_port = htons(9000); // 지역 포트번호 , 2바이트 
//    serveraddr.sin_addr.s_addr = htonl(INADDR_ANY); // 지역 IP 주소
...
//      {
//         // 데이터 받기
//         retval = recv( // 블로킹함수 , 데이터가 들어올때까지 블락, 종료하면 retval = 0
//                client_sock, //통신소켓핸들
//                buf, //받을 애플리케이션 버퍼
//                BUFSIZE, //수신 버퍼의 최대 크기
...
//
//      // closesocket()
//      closesocket(client_sock);// 소켓메모리 종료
//      printf("\n[TCP 서버] 클라이언트 종료: IP 주소=%s, 포트 번호=%d\n",
//         inet_ntoa(clientaddr.sin_addr), ntohs(clientaddr.sin_port));
//   }
//
