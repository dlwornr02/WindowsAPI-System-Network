...
//    case WM_CREATE:
//		tlsIndex = TlsAlloc();//동적 TLS사용법
//		hMutex = CreateMutex(NULL,FALSE,NULL); //커널오브젝트에서 진행되는 동기화이기때문에 cpu에 부담을 덜어준다.
//		//실행순서는 보장되지않는다.
//		break; 
//	case WM_LBUTTONDOWN:
//		ptp = new TParam;
...
//    case WM_CREATE:
//		tlsIndex = TlsAlloc();//동적 TLS사용법
//		hMutex = CreateMutex(NULL,TRUE,NULL); //커널오브젝트에서 진행되는 동기화이기때문에 cpu에 부담을 덜어준다.
//		//인자(상속여부,생성스레드에 소유여부,)
//		//실행순서는 보장되지않는다.
//		break; 
//	case WM_KEYDOWN:
                         ...
                         WPARAM wParam, LPARAM lParam) 
{
    ...
    case WM_CREATE:
		tlsIndex = TlsAlloc();//동적 TLS사용법
		//hMutex = CreateMutex(NULL,FALSE,NULL); //커널오브젝트에서 진행되는 동기화이기때문에 cpu에 부담을 덜어준다.
		hMutex = CreateMutex(NULL,FALSE,"myMutex_777"); //커널오브젝트에서 진행되는 동기화이기때문에 cpu에 부담을 덜어준다.
		//인자(상속여부,생성스레드에 소유여부,뮤텍스의 이름)
		//실행순서는 보장되지않는다.
		break; 
	case WM_KEYDOWN:
		switch(wParam)
